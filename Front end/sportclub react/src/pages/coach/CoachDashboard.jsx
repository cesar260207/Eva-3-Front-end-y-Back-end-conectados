...
const fetchRoutines = async () => {
  try {
    const response = await fetch(ROUTINES_API, { headers: getHeaders() });
    if (response.ok) {
      const result = await response.json();
      setRoutinesList(result.data || []);
    }
  } catch (error) {
    console.error("Error al cargar rutinas desde el servidor:", error);
  }
};
  ...
  if (!formData.clientName) {
    Swal.fire('Error', 'Por favor, seleccione un alumno válido.', 'error');
    return;
  }
  ...
  client_email: selectedUser ? selectedUser.email : "",
  ...
  try {
    const url = isEditing ? `${ROUTINES_API}/${currentRoutineId}` : ROUTINES_API;
    ...
    if (response.ok) {
      ...
      Swal.fire('Éxito', 'La planificación se ha guardado en la base de datos.', 'success');
    ...
    console.error("Error:", error);
    Swal.fire('Error', 'No se pudo guardar la rutina en el servidor.', 'error');
  }
};
  ...
  const handleDeleteRoutine = async (routineId, clientName) => {
    const result = await Swal.fire({
      title: '¿Está seguro?',
      text: `¿Desea eliminar la planificación de ${clientName}? Esta acción es irreversible.`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#dc3545',
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(`${ROUTINES_API}/${routineId}`, {
          method: 'DELETE',
          headers: getHeaders()
        });
        ...
        if (response.ok) {
          ...
          throw new Error(err.message || 'No se pudo eliminar la rutina');
        }
      } catch (error) {
        console.error('Delete error:', error);
        Swal.fire('Error', error.message || 'No se pudo eliminar la rutina', 'error');
      }
    }
  }

  // --- FUNCIONES ESTRUCTURALES ORIGINALES DEL ARCHIVO ---
