...
function AdminDashboard() {
  ...
  const [adminName, setAdminName] = useState(""); // Nuevo estado para identificar al admin

  const API_URL = "http://localhost:3000/api/users";

  // headers provided by authService.getHeaders()
  ...
  useEffect(() => {
    const checkSession = () => {
      ...
      if (!userSession || userSession.role !== "admin") {
        ...
        setAdminName(userSession.name); // Identificación del administrador
      }
    };
    checkSession();
  ...
  const handleDeleteUser = async (id, name) => {
    Swal.fire({
      ...
      text: `Esta acción eliminará permanentemente al usuario "${name}" del sistema.`,
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#dc3545",
      cancelButtonColor: "#6c757d",
      ...
      if (result.isConfirmed) {
        ...
        if (response.ok) {
          Swal.fire({
            title: "Eliminado",
            text: "El usuario ha sido removido de los registros con éxito.",
            icon: "success",
            confirmButtonColor: "#dc3545"
          });
          window.location.reload();
              ...
              {usersList.map((user) => (
                <tr key={user.id}>
                  ...
                  <td className="py-3 text-end px-4">
                    ...
                    <Button variant="danger" size="sm" className="fw-bold px-3 shadow-sm" onClick={() => handleDeleteUser(user.id, user.name)}>Eliminar</Button>
                  </td>
                </tr>
              ))}
            </tbody>
